...
function EditEmployeePopup({ employee, onSuccess, onClose }) {
  const [formData, setFormData] = useState({
    tenDangNhap: employee.tenDangNhap || '',
    hoTen: employee.hoTen || '',
    soDienThoai: employee.soDienThoai || '',
    vaiTro: employee.vaiTro || 'NhanVien',
  });
  ...
  const handleSubmit = async (e) => {
    ...
    setError(null);
    try {
      await employeeApi.update(employee.maNhanVien, formData);
      onSuccess();
    } catch (err) {
  ...
  return (
    <div className={popupStyles.overlay} onClick={onClose}>
      <div className={popupStyles.popup} onClick={(e) => e.stopPropagation()}>
        ...
        <h2 className={popupStyles.title}>Cập nhật thông tin nhân viên</h2>
